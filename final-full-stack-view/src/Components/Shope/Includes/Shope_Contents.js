...
function Shope_Contents() {
  ...
  */
  ...
  const cureentProduct = products_section2_shope.slice(indexOfFirstProduct, indexOfLastProduct);
  const pageNumber = [];
  for (let i = 1; i <= Math.ceil(products_section2_shope.length / ProductPerPage); i++) {
    pageNumber.push(i);
  }
  ...
  const pagination = (num) => {
    setcurrentPage(num);
     const pag_item__=document.querySelectorAll('.paginat_item_here');
          ...
          <ul class="pagination pagination-lg">
            {
              pageNumber.map(number => (
                <li class="page-item " aria-current="page" key={number}>
                  <span class="  paginat_item_here" onClick={pagination.bind(this, number)}>{number}</span>
                </li>
              ))
            }
